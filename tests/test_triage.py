...
def test_committed_projection_has_no_run_timestamp() -> None:
    artifact = Path(bt.TRIAGE_PATH).read_text(encoding="utf-8")
    assert '"generated"' not in artifact
...
def census(empty_bib_pages=()):
    return {
        "source": {
            "empty_content_pages": {
                "bibliographic_ns0": [
                    {"page_id": p, "title": "t", "text_id": "1"}
                    for p in empty_bib_pages
                ],
                "non_bibliographic": [
                    {"page_id": 2365, "namespace": 8, "title": "Print.css"}
                ],
            }
        }
    }
...
def test_false_negative_becomes_detectable_with_raw_value():
    results = [
        verify_result(
            87,
            {
                "translator_false_negative": {
                    "detected_in_raw": "Felix Braun",
