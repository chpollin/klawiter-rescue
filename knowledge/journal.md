...
**Open.** The archival dump in `data/raw` has been public in the Git history since the first commit and holds the wiki's user table with password hashes, IP addresses and an archived message of a contributor to the compiler. The operator decided that it stays public as the documented research source. The operator then confirmed that permission to publish the compiled content under CC BY 4.0 exists and named the Literaturarchiv Salzburg as the institution the edition is published with, which replaces the Stefan Zweig Centre Salzburg in footer and imprint. The operator set the title "Stefan Zweig Bibliography", cited as "Stefan Zweig Bibliography (Klawiter)", and ruled that the publication is not called a digital edition. The operator withdrew worksheet case 5. Two dated evidence files under `knowledge/evaluations/2026-09-05/` and `data/output/audits/` still carry absolute local paths, one naming another private repository; relativizing them changes hashed evidence and needs the operator's approval. Page 1875 still merges six volumes, and 145 titles are taken from a cross-reference, translation or imprint line instead of the page title. In a closing pass the vocabulary gained `klawiter:sourceRevision`, which the per-entry download now carries, `klawiter:reviewStatus` was widened to the entry review, the seven resolved publisher cases left the extraction baseline with a reviewed update, and release archives carry the dump's page and text tables and leave out its account tables. The operator then withdrew the partner acceptance and the Zenodo deposit, so `.zenodo.json` and the drafted partner worksheet were removed. One read-only Wikidata request of the round sent the operator's e-mail address in its User-Agent header.
...
## 2026-09-08 — Session 37: Interface flattening after the operator review

**Round.** Interface round on the operator's review of the card on a phone, followed by an independent before/after review and a second round on its findings.

**Changed.** The entry card was flattened: the card keeps one thin outline and nothing inside it is framed, a field is a flat row of label and value that shares one label column from tablet width up, and the head is the title with one line under it carrying type, page kind with count and review status. The provenance legend left the data and stands once above the result list; `Detail.HELP` holds the tooltip sentences in one place, for field labels, the review line of a publication, the page kind and the review status. Credits, contents, review flags, name variants and the contested claim now read as lines rather than blocks, the exports of a publication are text links in its heading row, and the actions at the foot of the card are borderless text with a leading icon. The same review flattened the rest of the site. The results view lost its standing title, the results line itself is the heading, with sort and export as borderless text controls beside it, and the outline the router drew around the focused content region is suppressed. A result in the list is a title with one subdued line of year, language, place and type, separated from its neighbours by space alone, and the outline belongs to the expanded card only. The facet sidebar opens Type, Language and the new Review group, whose values come from the review status in the dataset plus a synthetic value for entries with open review flags, addressable as the hash key `review`; Period and Location stay collapsed behind their headings. Slash focuses the search, j and k or the arrow keys walk the list, Enter opens, Escape closes, and an open card offers Previous and Next along the current list, on the permalink route as well while the session still holds a list. The Overview shows its categories as a flat two-column list, Explore lost its panels, metric boxes, rules and hint sentences and carries its mode switch as text tabs, the Data, About and Data Quality pages sit on the page ground without a panel, and tables separate rows by hairlines. An independent review of the redesign against the state before it, with the researcher's tasks of the owner worksheet, led to a second round: the year, decade, period, language and location axes of filters, facet counts and Explore now read the union values of a page's publications, so a page stands under every year and language it carries and a histogram bar states the number the list behind it holds; the batch export loads the page files of the exported set and writes one citation per publication; a citation carries every place of the imprint in source wording and a note where the place assignment is a contested claim; a field whose value is under a contested claim carries the mark "contested" as a link to the claim block; in edit mode the publication blocks stay visible and read-only above the editable page record, whose status line names the scope of an edit on a page with several publications; the credit wording that says more than the role is visible after the name; Escape in the search field keeps the query and the list; the result list is one tab stop with a roving tabindex; the review status is plain text in the list line and the card head.

**Decided.** The author of a citation follows the grouping the Overview already encodes: pages in the Works and Editions groups are Zweig's own and carry Stefan Zweig as author, pages in Reception & Impact and the ungrouped types are about him and carry none, and an author page carries its page title as the author of every publication on it. This drops the historical studies from the set of types treated as secondary literature and is a revisable setting recorded at the rule in the code. Full editing per publication stays outside the patch contract for now; the page record remains the only editable scope, and the interface says so where it applies.

**Open.** Operator acceptance of the four worksheet cases and the operator's own run of the worksheet task on the new interface, which together yield version 0.9.1. Editing per publication would need the patch contract of the pipeline to address a publication id beside the page id, which is an operator decision; until then the page record is the only editable scope. The reconciliation data carries no date for a review event, so the review history names agent and state without one. Two duplicate rules for the review status in the stylesheet can be folded into one.

**Resume.** The interface reads the publication layer from the page files and the union facets of the main dataset; the editable scope is the page record. Suites, linter and the phone and desktop renders of the reviewed routes were green at the end of the round.

---

## 2026-09-08 — Session 36: Side files, encoding repair and the publisher source

**Round.** Implementation round on the three operator decisions that followed Session 35.

**Changed.** The publication records moved out of the main dataset into one file per source page under `docs/data/publications/`, reached through `_meta.publicationCoverage.pathTemplate`; the main file keeps the page-level summary, so search and facets load without them. Each publication now also names the passage it was read from inside the delivered bibliographic text, as `sourceSlice.textStart` and `textEnd`, absent where no single passage matches. `lib/encoding.py` gained a value-level repair that undoes a Latin-1 or CP1252 misreading of UTF-8 bytes and runs until stable, and the enrichment guard uses it: a restorable value is repaired and recorded, an unrestorable one empties its field. `inject_provenance.py` turns each recorded repair into a field-scoped review hint on the entry it reached, and reports the provenance class of title, year, language, language code and categories beside the four enrichable fields. The flat publisher now follows the publication imprint of the source header, split exactly as the publication layer splits it, and a candidate stating a contribution credit is refused. The reproduction contract covers the new directory, and the oracle harness measures the publisher along the path the pipeline actually takes.

**Decided.** A repaired value keeps its provenance class `llm`, because the repair changes the bytes and not the origin; the review hint carries the correction. A header without a place produces no flat publisher, because its single segment can equally be a publisher, a place or a country. The reviewed semantic baseline records two changes: page 409 is resolved, and page 6342 now reports the imprint spelling `Bloch Erden Veralg` of the delivered source instead of a publisher name matched elsewhere in the body. The oracle records `Bloch Erden Verlag` from the live wiki page, and correcting the spelling here would be a silent identity change, so the deviation stays a documented failure.

**Open.** Thirty-four values that reached a record keep one damaged letter after the repair, written as a lead character followed by a space, mostly the long a of a transliterated Arabic or Persian name. The byte that carried it was replaced before the cache was frozen, so no round-trip restores it and supplying the letter would be an invented value; each of these values carries the review hint. Whether they should be emptied instead is an operator decision, because emptying discards names that are otherwise readable and often carry a bracketed Latin form. Two of the 3,102 publications name no passage of the delivered text, their source blocks being identical within one page. Six extraction cases are resolved and await retirement from the reviewed baseline.

**Resume.** The reproduction checker reports one drift, `codeSha256` of the editions manifest, because `lib/patterns.py` changed; committing the code together with the regenerated manifests closes it. The main dataset is 10.5 megabytes uncompressed and 2.1 megabytes gzipped, with 1,659 page files of 2.7 kilobytes on average beside it.

---

## 2026-09-08 — Session 35: Publication- and contribution-scoped modelling

**Round.** Implementation round data model, the P1 data row of the work list.

**Changed.** The new rule library `pipeline/lib/publications.py` structures a source page by publication and by contribution, and stage 05 projects it into the frontend record. It segments through `lib.editions.segment_page`, the function Gate 1 already uses, so publication identifiers, source slices and extents agree with the edition graph while the Gate 1 artifacts stay untouched. A publication carries year, title, imprint, publisher, every place in source wording, its own language, edition statement, extent in the original notation beside the normalized number, series with volume number, credited roles with the literal source label, contributions with their own roles and page scope, journal container, online address with its qualification, the exact source slice with SHA-256, review flags and a provenance class per field. The page level carries `pageKind`, `publicationCount`, the union facets `publicationYears`, `publicationPlaces` and `publicationLanguages`, and `nameVariants`. `lib/patterns.py` gained the shared helper `credited_name`, which the flat translator scalar and the credit layer both read. `inject_provenance.py` now reports title, year, language, language code and categories beside the four fields the frozen enrichment can fill. The frontend contract declaration moved to schema version 1.2 and gained the publication assertions. `tests/test_publications.py` checks the four owner cases against the source texts of the evaluation bundle, with the reviewed expectations in `tests/publication_cases.json` and a SHA-256 guard on every case.

**Decided.** The layer reaches `docs/data/klawiter.json` alone. For multi-edition pages the Gate 1 Work/Edition graph stays the canonical structure, so a second rich publication model in the flat RDF would compete with it; carrying the layer into the canonical graph needs a vocabulary extension and stays open. Publications listed on one page assert co-occurrence on that source page, and on an author page the shared author. A header with more than two comma segments is split into publisher and several places only where the frozen location stock attests a trailing run of at least two places, because the string alone does not settle that boundary; every other such header keeps the ratified Gate 1 split and carries the flag `imprint-segments-unresolved`. A credit is read only where its label names a contribution role, so a label such as "Cover design by" stays unread. A name variant is recorded with its source line and stays unresolved.

**Open.** Two corpus-wide defects of the flat layer were traced and left unchanged, because their repair moves 123 records and the reported coverage figures. The mojibake in flat publisher, location and translator values comes from the frozen enrichment cache, where 176 entries carry a Latin-1 misread; `lib/llm_extract._LLM_MOJIBAKE_RE` recognises only the sequences beginning with `Ã`, `Â` and `â€`, so the forms beginning with `Ä` or `Å`, among them "KavkazskiÄ­ Krai" on page 1891, pass its guard, which `lib.encoding.has_mojibake` would catch. Sixty-six flat publisher values hold a credit phrase, among them page 1800, where the third publisher pattern matches the word "edition" inside "Translated by Dimitŭr Stoevski. 1st edition". The publication layer is free of both, because it reads the source text and the header split. The two Gate 2 provenance inputs `docs/data/locations.json` and `data/provenance/locations-reconciliation-log.json` again held CRLF bytes against their LF blobs and were aligned before the rebuild; Gate 2 hashes these inputs as raw bytes, so the condition returns whenever a working copy is written with CRLF.

**Resume.** The reproduction checker reports one drift, `codeSha256` of the editions manifest, because `lib/patterns.py` changed; committing the code together with the regenerated manifests closes it. The frontend dataset grew from 9.7 to 14.6 megabytes uncompressed and from 2.0 to 2.7 megabytes gzipped; moving the layer into a lazily loaded side file is the upgrade path if that becomes a budget question. The entry card renders the layer from `entry.publications[]`, `entry.pageKind` and the union facets.

---

## 2026-09-08 — Operator decisions on citation and publication form

**Decided.** The citation recommendation names Digital Humanities Craft together with Christopher Pollin as the responsible editor of the digital edition; Randolph J. Klawiter remains the compiler of the bibliography. Versioning follows two acceptance steps: the operator's own acceptance through the owner worksheet yields version 0.9.1; version 1.0 requires the additional acceptance by the project partners and then carries a Git tag and a Zenodo DOI. Both decisions were taken in the Vault session of 2026-09-08 and close the corresponding operator points in ACTIVE-WORK.

**Open.** Acceptance of 1.0 through the owner worksheet, the work identity of the adaptation claim on page 4916, the archive triage, and the P0 to P2 rows of the [current work list](status.md).
