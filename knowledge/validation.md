...
The earlier encoding repair removed the gross mojibake at the field level (no replacement characters remained in title, publisher, location, translator), but transliterated titles still carried double-encoded artifacts. Entry 804's title read "Mardkutâyan asteghayin zhamerÄ", where the â is a double-encoded transliteration apostrophe and the Ä a double-encoded diacritic. The cause: `fix_mojibake` only triggered on the Ã/Â (C2/C3) lead bytes, so the Latin Extended-A and Extended-Additional diacritics of romanized Arabic, Greek, Vietnamese and Slavic titles (ā, ī, ş, ḥ, ṭ) and the double-encoded smart quotes were never reached. This matched the roughly 345 titles the heuristic tests already bound ([[testing#f-semantic--is-the-value-correct]]).

### Repair broadened in code, measured against a committed artifact

`fix_mojibake` (`pipeline/lib/encoding.py`) now repairs each mojibake run independently: it re-encodes the run as Latin-1 and decodes as UTF-8, which inverts the corruption for 2-, 3- and 4-byte sequences alike. The repair is self-validating, a run whose bytes are not valid UTF-8 once re-encoded is left untouched, so a clean accented letter before a Latin-1 guillemet (Catalan "nació»") is not corrupted; `has_mojibake` now reports only repairable runs, so the validator no longer flags such clean text. The change is locked by `tests/test_encoding.py` (`TestMojibakeTransliteration`), including a clean-German guard and an idempotency check.

Measured by `pipeline/measure_mojibake_repair.py` over all namespace-0 records of `01_extracted.csv`, written to the committed `data/output/mojibake-repair-report.json` (deterministic). Of 62,351 detected runs, all 62,351 are repaired, 0 are self-rejected, the repair is idempotent, and after repair 0 records carry a repairable residual. The full list of repaired runs whose output falls outside the common Latin, Greek, Cyrillic, Hebrew and Arabic blocks is in the report for review: it contains 5 entries, all legitimate (four polytonic Greek letters and a bullet), which confirms the broad signature did not corrupt clean text and that a block-whitelist guard would have been wrong, since it would have rejected those five.

As with the location fix, this is landed in the pipeline code but not yet in the published frontend, which regenerates only on the next full run. The title is still not an editable field; that is a separate Strand-2 milestone. The repair restores fidelity the pipeline itself had introduced; it does not edit the source.
