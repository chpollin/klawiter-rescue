...
`edit.js` (localhost-only, gated by `App.state.isLocal && App.state.editMode`) now implements increments 1 to 3. Inline editing of the four provenance-tracked fields (publisher, location, translator, pageCount) is typed as one of the three actions Accept / Correct / Add, every action carries the v2 edit-history shape (`action`, `oldValue`, `newValue`, `previousProvenance`, `edited_by` role, `edited_at`, `source`), the three-status review is surfaced per entry as a chip raised to Mensch-geprüft by pending human edits, pending edits persist in `localStorage` across reloads, and Save downloads a `patchVersion: 2` document that `pipeline/apply_patches.py` consumes directly. The frontend/backend patch contract is pinned by `tests/test_patch_contract.py`. `.github/workflows/validate-patch.yml` validates patches on pull requests. The dataset-side write-back is `pipeline/apply_patches.py` (Session 17, unit-tested): it applies approved corrections as an overlay, sets the corrected field's provenance to `editor`, preserves the machine original in an edit history, and raises the review status.

Increment 2 (Session 21) bundles the existing signals into per-entry attention hints: `pipeline/build_triage.py` reduces the committed `verification-report.json` and `census-report.json` to `docs/data/triage.json` (flag shapes pinned by `tests/test_triage.py`), and `edit.js` folds those flags together with the entry's provenance layers into an ordered hint list (census anomaly, value not found in raw text, value detectable but not extracted, llm, missing) shown as a Prüfhinweise block per entry, a compact chip per result card, a per-field marker, and an edit-mode-only "Prüfbedarf zuerst" sort. The ordering follows the signal-class priorities in [[production-readiness#provenienz-schichten-als-verifikationsgrundlage]]; nothing numeric is displayed or derived, in line with the protocol-not-instrumentation frame. Increment 3 (Session 21) shows the source evidence beside each tracked field: the passage of the entry source holding the machine value (or the verify.py-detected raw value for a missing field), whitespace-tolerant and case-insensitive with the match highlighted and a multi-occurrence count that surfaces multi-edition ambiguity; where no field-precise span is derivable, the field falls back to the collapsible full source, the honest variant. The matching and ordering logic is pinned by `tests/evidence_triage.test.js` (run via `tests/test_frontend_logic.py`). What remains is increment 4's live local write-back endpoint that lets the frontend apply corrections without the manual patch-file step, and increment 5's protocol read-out.
...
2. **Uncertainty surface** (done, Session 21): provenance classes, verify.py flags, and the census anomaly are bundled into per-entry attention hints (`build_triage.py` → `triage.json` → hint list, card chip, field marker, Prüfbedarf sort), verified in the browser on localhost and pinned by `tests/test_triage.py` and `tests/evidence_triage.test.js`. The remaining calibration input is the stratified per-field spot-check from [[validation#method-and-limits]]; until it lands, the class ordering follows the provenance-layer priorities in [[production-readiness]].
3. **Editing scope** (done for the tracked fields, Session 21): source evidence is shown beside each of the four provenance-tracked fields, field-precise where a span is derivable, collapsible full source where not. Extending inline editing beyond the four tracked fields (title above all, see [[validation#implications-for-the-editing-tool]]) stays open and is tied to the multi-edition gate question.
...
Increments 4 and 5 touch the pipeline and the provenance model; increment 4 hangs on the operator's Modellweg gate question in [[production-readiness#braucht-den-operator]].
