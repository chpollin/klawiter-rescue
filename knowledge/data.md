...
### EQUALIS Metrics (Planned)

The EQUALIS framework (Evaluating Quality and Explainability in AI for the Humanities) from the DIA-XAI project will measure the EIL verification workflow on this dataset. The Interaction dimension (EQUALIS-I) uses ratio-based metrics, not time-based measurements:

- **Accept/Correct/Add ratio** — overall and by provenance (regex/llm/missing). If 80% Accept: pipeline is solid. If 40% Accept, 50% Correct: pipeline needs work
- **Correction rate by provenance** — how often are `regex` fields corrected vs. `llm` fields vs. `missing` fields? Measures reliability of each extraction method directly
- **Correction distribution by entity type and field** — do fiction entries need more corrections than secondary literature? Is publisher the problem field or translator?
- **Ratio shift across iterations** — after a pipeline improvement, does the Accept ratio increase? Measures Learning without a time component

These metrics fall out as a byproduct of editor actions in the EIL interface — every Accept/Correct/Add is logged with field, provenance, and entry type. No controlled experiment or time tracking required. See [[about#dia-xai-connection]] for the project context.
