...
Das statische Frontend ist live, autark (alle Bibliotheken und Schriften lokal vendoriert, kein externer Request beim Laden) und trägt Volltextsuche, Facetten, Explorationssichten und Export. Die Editierschicht `edit.js` ist localhost-gated und implementiert Increment 1 aus [[eil-editing]]: Inline-Editieren der vier provenienz-getrackten Felder, getypt als Accept, Correct oder Add, mit v2-Edit-History, Drei-Status-Review je Eintrag, Persistenz in `localStorage` und Save-Export eines Patch-v2-Dokuments, das `pipeline/apply_patches.py` liest. Der Frontend-Backend-Patch-Kontrakt ist durch einen Test gepinnt. Offen sind Increment 2 (kalibriertes Triage-Signal), Increment 3 (per-Feld-Rohtext neben jedem Feld) und Increment 4 (lebender lokaler Write-Back-Endpunkt statt manuellem Patch-Datei-Schritt).
