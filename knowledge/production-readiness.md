...
| Public release | curated source package, consistent metadata, explicit release scope and acceptance | release scope set on 22 and 23 September (see below); `CITATION.cff`, `.zenodo.json`, licence texts, third-party notices and the source inventory aligned on 23 September, release archives leave out only the account tables `zweig_part_03.sql`, and `.zenodo.json` assigns the record to the Digital Humanities Craft community on Zenodo; rights basis confirmed and institution named by the operator on 23 September; the dump stays in the public Git history and the title is set by operator decision; the acceptances remain |
