...
USER_AGENT = 'KlawitterBibliography/1.0 (https://github.com/chpollin/klawiter-rescue; research)'
...
def pick_best_match(hits_by_lang: dict) -> dict | None:
    ...
    all_hits.sort(key=lambda h: (h.get('match', False), h.get('score', 0)), reverse=True)
    return all_hits[0]
...
def main():
    ...
    for lang in ['en', 'de']:
        ...
        for bi, batch in enumerate(batches):
            print(f'  Batch {bi + 1}/{len(batches)} ({len(batch)} names)...', end=' ', flush=True)
            try:
    ...
    # Log unmatched
    for key in locations:
        if key not in matches and not any(l['location'] == key for l in log):
    ...
    # Write results
    LOCATIONS_PATH.write_text(json.dumps(locations, indent=2, ensure_ascii=False), 'utf-8')
    ...
    unmatched = [l for l in log if l['status'] == 'unmatched']
    low_score = [l for l in log if l['status'] == 'low_score']
    if low_score:
        print(f'\n  Low score ({len(low_score)}, need review):')
        for l in low_score[:10]:
            print(f'    {l["location"]} → {l["wikidataId"]} ({l["wikidataLabel"]}) score={l["wikidataScore"]}')
    if unmatched:
